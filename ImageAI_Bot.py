...
def handle_message(message):
    ...
    # Определяем, хочет ли пользователь запросить изображение
    if user_input.lower().startswith("изображение:"):
        image_prompt = user_input[len("изображение:"):].strip()
        try:
            ...
            bot.reply_to(message, f"Не удалось создать изображение: {str(e)}")
    else:
